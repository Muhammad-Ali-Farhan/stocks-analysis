...
def update_bollinger_bands_graph(symbol):
    ...
    trace_close = go.Scatter(x=df.index, y=df['Close'], mode='lines', name='Close Price')
    trace_upper = go.Scatter(x=df.index, y=df['Upper Band'], mode='lines', name='Upper Band', line={'dash': 'dash'})
    trace_lower = go.Scatter(x=df.index, y=df['Lower Band'], mode='lines', name='Lower Band', line={'dash': 'dash'})

    return {
        'data': [trace_close, trace_upper, trace_lower],
        'layout': go.Layout(title=f"{symbol} Bollinger Bands", xaxis={'title': 'Date'}, yaxis={'title': 'Price'})
    }
...
def update_price_change_graph(symbol):
    ...
    trace_price_change = go.Scatter(x=df.index, y=df['Price Change %'], mode='lines', name='Price Change %')
...
def update_rsi_graph(symbol):
    ...
    trace_rsi = go.Scatter(x=df.index, y=df['RSI'], mode='lines', name='RSI')

    return {
        'data': [trace_rsi],
        'layout': go.Layout(title=f"{symbol} RSI", xaxis={'title': 'Date'}, yaxis={'title': 'RSI'})
    }
...
def update_macd_graph(symbol):
    ...
    return {
        ...
        'layout': go.Layout(title=f"{symbol} MACD", xaxis={'title': 'Date'}, yaxis={'title': 'MACD'})
    }
